      ...
      <div v-show="islogin" class="loginbox" :class="{ loginboxinregiest: registerState }">
        ...
        <div class="inputbox">
          <transition-group name="inputboxesT" appear>
            ...
            <div key="1" class="inputboxes" v-show="normalState">
              用户名:<input type="text" required v-model="userNameState1" maxlength="24" />
              <hr />
              密 码:<input type="password" required v-model="userPasswordState1" maxlength="32" />
            </div>
            <!-- QQ快速登录box -->
            <div key="2" class="inputboxes" v-show="QQstate">
              <span class="backward" @click="backward">◀</span>
              QQ号:<input type="text" required v-model="userNameState2" maxlength="10" />
              <hr />
            </div>
            <!-- 注册box -->
            <div key="3" class="inputboxes" v-show="registerState">
              ...
              <hr />
              邮 箱:<input type="email" required v-model="userEmailState3" maxlength="32" />
              <br />
              密 码:<input type="password" required v-model="userPasswordState3" maxlength="32" />
              <br />
              验证码:<input type="text" required v-model.number="userCodeState3" maxlength="6" />
              <button @click="verfiryCode" :style="{ zIndex: verfiryButtonShow }">验证 {{ verfirytime }}</button>
              <br />
            </div>
            <!-- 忘记密码box -->
        ...
        <div class="options">
          <transition-group name="optionsT" appear>
            ...
            <button key="3" v-show="false" @click="QQquickLogin" disabled>QQ快速登录(暂停使用)</button>
            <button key="4" v-show="isoption" @click="fogetPassword">忘记密码</button>
            <button key="5" v-show="isoption" @click="register">用户注册</button>
          </transition-group>
        </div>
      </div>
    </transition>
...
export default {
  ...
  methods: {
    // vuex全局状态
    ...mapActions('userInfo', ['uploadUserInfo', 'uploadUserSetting', 'saveAllusers', 'findSpaceWith']),
    ...
    verfiryCode() {
      ...
      if (this.userNameState3 == '' || this.userPasswordState3 == '' || this.userEmailState3 == '') {
        this.smileShake()
        ...
        this.$axios.post('/api/userRegisterCode', sendData).then(
          (response) => {
            ...
            // 得到了响应,仅取消loading旋转
            this.responseMessage('已发送,请注意查看邮箱,60s后才可以再次发送验证码')
            // 按钮不可点击
            this.verfiryButtonShow = -1
            // 倒计时
            this.verfirytime = 60
    ...
    async mailNotice(toQQ, sendMail) {
      ...
      await this.$axios.post('/api/loginNoticesAdmain', { userQQ: toQQ, userSign: new Date(parseInt(Date.now())).toLocaleString().slice(5), userEmail: sendMail }).then(
        (response) => {},
        (error) => {
    ...
    getUserInfo(userQQ, state) {
      this.$axios.post('/api/getUser', { userQQ: userQQ }).then(
        (response) => {
          // 向store中加入用户的个人信息
          if (state == 'login') {
            this.mailNotice(userQQ, response.data.userEmail)
          }
      ...
      this.$axios.post('/api/userRegister', data4).then(
        (response) => {
          ...
          if (response.data == -1) {
            ...
            // 注册SpaceWith
            this.$axios.post('/api/insertSpaceWith', { userQQ: this.userNameState3 }).then(
              // eslint-disable-next-line no-unused-vars
              (response) => {
            ...
            // 注册picwith
            // eslint-disable-next-line no-unused-vars
            this.$axios.post('/api/insertPicWith', { picWithQQ: this.userNameState3 }).then(
              (response) => {},
              (error) => {
                console.log(error.message)
              }
    ...
    logining() {
      ...
      if (this.loginWhere == 4) {
        // 若未输入东西
        if (this.userNameState3 == '' || this.userPasswordState3 == '' || this.userEmailState3 == '' || this.userCodeState3 == '') {
          this.smileShake()
      ...
      if (this.loginWhere == 2) {
        ...
        if (this.userNameState2 == '') {
          ...
          // 向QQ发送请求
          this.$axios.get('https://api.usuuu.com/qq/' + this.userNameState2).then(
            (response) => {
              // 登录不成功
...
.enterbuttonT-leave-active {
  animation: puff-out-center 8s cubic-bezier(0.165, 0.84, 0.44, 1) both;
}
