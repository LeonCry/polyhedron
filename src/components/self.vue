...
<template>
  <transition name="selfboxT">
    <div class="selfbox">
      <!-- 背景 -->
      <div class="myback">
        <img v-if="this.user.userBack" :src="`/polyhedronX/HeadsAndBacks/Backs/${this.user.userBack}`" alt="背景" />
      </div>
      <!-- 头像网名 -->
      <div class="myhead">
        <img v-if="this.user.userHead" :src="`/polyhedronX/HeadsAndBacks/Heads/${this.user.userHead}`" alt="头像" />
        <br />
        <!-- 网名 -->
...
.selfboxT-leave-active {
  animation: swing-in-top-fwd 0s cubic-bezier(0.175, 0.885, 0.32, 1.275) both reverse;
}
/* 好友个体进入退出动画 */
