<template>
  <div class="chatSearchResultBox">
    <!-- 头像 -->
    <div class="heads">
      <img v-if="resultProps.userHead" :src="`/polyhedronX/HeadsAndBacks/Heads/${resultProps.userHead}`" alt="头像" />
    </div>
    <!-- 名称\时间与聊天记录 -->
    <div class="rightMore">
      <div class="nameAndTime">
        <span
          >{{ resultProps.userName }} <span v-show="resultProps.remakeName != ''"> ({{ resultProps.remakeName }})</span></span
        >
        <span> {{ new Date(parseInt(resultProps.chatTime)).toLocaleString().slice(5) }}</span>
      </div>
      <div class="Chats">
