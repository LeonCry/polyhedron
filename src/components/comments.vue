...
<template>
  <div class="commentsbox">
    <!-- 头像名字评论时间回复按钮 -->
    <div class="myhead">
      <img v-if="comment" :src="`/polyhedronX/HeadsAndBacks/Heads/${comment.user.userHead}`" alt="头像" />
      <!-- 网名和发表时间 -->
      <div class="usernametime">
        <span
          ><span v-show="remakeName">({{ remakeName }}) </span>{{ comment.user.userName }}<span>#{{ comment.commentFloor }}楼</span></span
        >
        <span>{{ new Date(parseInt(comment.commentTime)).toLocaleString().slice(5) }}</span>
...
export default {
  ...
  created() {
    ...
    this.$axios.post('/api/selectReplyBySpaceIdAndTargetQQ', { replySpaceId: this.comment.commentSpaceId, replyCommentFloor: this.comment.commentFloor, pageStart: 0, pageEnd: 9999 }).then(
      (response) => {
  ...
  methods: {
    ...
    getRemakeName(userQQ, friendQQ) {
      this.$axios.post('/api/getOneFriends', { userQQ: userQQ, friendQQ: friendQQ }).then(
        (response) => {
    ...
    async reply() {
      ...
      await this.$axios.post('/api/selectReplyBySpaceIdAndTargetQQ', { replySpaceId: this.comment.commentSpaceId, replyCommentFloor: this.comment.commentFloor, pageStart: 0, pageEnd: 9999 }).then(
        (response) => {
      ...
      if (this.replyCommentContent == '') {
        this.$bus.$emit('spaceNotice', false, '回复内容不可为空!')
      } else {
        let data = { replySpaceId: this.comment.commentSpaceId, replyQQ: this.user.userQQ, replyContent: this.replyCommentContent, replyTargetQQ: this.comment.commentQQ, replyCommentFloor: this.comment.commentFloor, replyTargetFloor: this.comment.commentFloor, isReplyComment: 1, myFloor: layer + 1, replyTime: Date.now() }
        this.replyCommentContent = ''
        // eslint-disable-next-line no-unused-vars
        await this.$axios.post('/api/insertReply', data).then(
          (response) => {
    ...
    async replyNotice() {
      ...
      // eslint-disable-next-line no-unused-vars
      await this.$axios.post('/api/addOneNotice', { sendUserQQ: this.user.userQQ, receiveUserQQ: this.commentProps.commentQQ, noticeType: 0, remarks: '回复了你的评论.' + 'Q-v4jvy-Q' + JSON.stringify(this.spaceProps), noticeTime: Date.now() }).then(
        (response) => {
...
.writecommentT-enter-active {
  animation: slide-in-blurred-left 0.45s cubic-bezier(0.23, 1, 0.32, 1) both;
}
.writecommentT-leave-active {
  animation: slide-in-blurred-left 0.35s cubic-bezier(0.23, 1, 0.32, 1) both reverse;
}
