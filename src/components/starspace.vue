        ...
        (response) => {
          if (response.data.length == 0) {
            ...
            response.data.forEach((friend) => {
              // 对每一个朋友查询其动态
              this.$axios.post('/api/selectSpaces', { publishQQ: friend.friendQQ, pageStart: 0, pageEnd: 9999 }).then(
                (response) => {
    ...
    async myGooder() {
      ...
      if (this.spaceWith['goods'].length == 0) {
        ...
        for (let index = 0; index < this.spaceWith['goods'].length; index++) {
          ...
          await this.$axios.post('/api/selectSpacesById', { publishId: goods, pageStart: 0, pageEnd: 9999 }).then(
            (response) => {
  ...
  mounted() {
    ...
    this.$bus.$on('spaceappear', (data1, data2, friendUser) => {
      ...
      if (data1) {
        ...
        this.$axios.post('/api/selectSpaces', { publishQQ: this.spaceUser.userQQ, pageStart: 0, pageEnd: 10 }).then(
          (response) => {
            ;(this.spaceSum = []), this.$bus.$emit('spaceLoading', false, '加载动态..')
            response.data.forEach((space) => {
      ...
      this.backgroundImg = { background: 'url(' + `/polyhedronX/HeadsAndBacks/Backs/${this.spaceUser.userBack}` + ') no-repeat center / cover' }
      this.backReadyFun()
    })
    // 接收来自其他窗口的数据,进行高度改变
