  ...
  <div v-if="space" class="spaceitembox">
    ...
    <div class="myhead">
      ...
      <div class="usernametime">
        <span
          ><span style="color: pink" v-show="remakeName">({{ remakeName }}) </span>{{ space.user.userName }}</span
        >
        <span>{{ new Date(parseInt(space.publishTime)).toLocaleString().slice(5) }}</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    async thinkNotice(msg) {
      ...
      await this.$axios.post('/api/addOneNotice', { sendUserQQ: this.user.userQQ, receiveUserQQ: this.spaceProp.publishQQ, noticeType: 0, remarks: msg + 'Q-v4jvy-Q' + JSON.stringify(this.spaceProp), noticeTime: Date.now() }).then(
        (response) => {
    ...
    activeRequest(operation) {
      ...
      let custom = []
      if (this.space[operation] != '') {
        custom = this.space.collector.split(',')
      }
      ...
      // eslint-disable-next-line no-unused-vars
      this.$axios.post('/api/updateSpace', data).then(
        (response) => {
          if (operation != 'sharer') {
    ...
    negetiveRequest(operation) {
      ...
      if (custom.includes(this.user.userQQ)) {
        custom.remove(this.user.userQQ)
      }
      // 构建传送参数json,
    ...
    spaceWithAdd(operation) {
      ...
      // eslint-disable-next-line no-unused-vars
      this.$axios.post('/api/updateSpaceWith', spaceWithString).then(
        (response) => {},
        (error) => {
          console.log(error.message)
        }
    ...
    async comment() {
      if (this.commentContent == '') {
        this.$bus.$emit('spaceNotice', false, '评论内容不可为空')
      } else {
        // 先发送请求返回该ID下的评论,查看自己是第几楼
        let floor = 0
        await this.$axios.post('/api/selectCommentBySpaceId', { commentSpaceId: this.space.publishId, pageStart: 0, pageEnd: 9999 }).then(
          (response) => {
        ...
        this.commentContent = ''
        // eslint-disable-next-line no-unused-vars
        this.$axios.post('/api/insertComment', data).then(
          (response) => {
    ...
    async commentNotice() {
      ...
      this.socket.send(JSON.stringify({ from: this.user.userQQ, to: this.spaceProp.publishQQ, message: message }))
      // eslint-disable-next-line no-unused-vars
      await this.$axios.post('/api/addOneNotice', { sendUserQQ: this.user.userQQ, receiveUserQQ: this.spaceProp.publishQQ, noticeType: 0, remarks: '评论了你的动态.' + 'Q-v4jvy-Q' + JSON.stringify(this.spaceProp), noticeTime: Date.now() }).then(
        (response) => {
...
.showpeopleT-leave-active {
  animation: swing-in-left-fwd 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) both reverse;
}
...
.sharepeopleT-leave-active {
  animation: swing-in-right-fwd 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) both reverse;
}
