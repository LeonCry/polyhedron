<template>
  <transition name="DetailT" appear>
    <div class="fooderCreater" ref="fooderCreater" v-show="isCreated">
      ...
      <img class="imgs" src="https://img.shetu66.com/2022/10/16/1665933915984154.jpg" alt="" />
...
export default {
  ...
  methods: {
    ...
    async confirms() {
      ...
      setTimeout(() => {
        if (this.isNewCreated) {
          ...
          this.$axios.post('/api/insertFoodOrders', { orderPerson: this.visitorId, orderDiners: this.pnums, orderContent: '', orderStatus: '点菜中', orderTime: Date.now() }).then(
            (response) => {
        ...
        // 如果是返回来修改的订单
          ...
          //更新订单
          this.$axios.post('/api/updateFoodOrders', { orderId: this.hasOrderId, orderDiners: this.pnums }).then(
            (response) => {
              this.uploadOrderId(response.data[0].orderId)
              this.uploadOrderUser(response.data[0].orderPerson)
    ...
    admin() {
      if (!this.isAdmin) {
        ...
        if (this.password == '') {
          ...
          this.$axios.post('/api/userLogin', { userQQ: 'SYSTEM', userPassword: this.password }).then(
            (response) => {
              if (response.data == -1) {
    ...
    // 先请求数据库,看该顾客是否有正在订单中
    isOrdering() {
      this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '点菜中' }).then(
        (response) => {
          if (response.data.length == 0) {
            // 没有正在点菜的订单,看是否有已下单的订单
            this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '已下单' }).then(
              (response) => {
                if (response.data.length == 0) {
                  // 没有已下单的订单,看是否有正在烹饪的订单
                  this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '烹饪中' }).then(
                    (response) => {
                      if (response.data.length == 0) {
    ...
    async isChangeOrder() {
      await this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '点菜中' }).then(
        (response) => {
          if (response.data.length == 0) {
            // 没有正在点菜的订单,看是否有已下单的订单
            this.isChangeOrder2()
    ...
    async isChangeOrder2() {
      await this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '已下单' }).then(
        (response) => {
          if (response.data.length == 0) {
            // 没有已下单的订单,看是否有正在烹饪的订单
            this.isChangeOrder3()
    ...
    async isChangeOrder3() {
      await this.$axios.post('/api/selectFoodOrdersByNameAndStatus', { orderPerson: this.visitorId, orderStatus: '烹饪中' }).then(
        (response) => {
          if (response.data.length == 0) {
            // 说明没有正在进行的订单,该页面需要进行显示
...
.DetailT-leave-active {
  animation: slide-top 0.4s 1.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
.DetailT-1-enter-active {
  animation: tilt-in-right-1 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
.DetailT-1-leave-active {
  animation: tilt-in-right-1 0.4s 1s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
.DetailT-2-enter-active {
  animation: slide-top 0.4s 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
.DetailT-2-leave-active {
  animation: slide-top 0.2s 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
.DetailT-3-enter-active {
  animation: slide-top 0.4s 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
.DetailT-3-leave-active {
  animation: slide-top 0.2s 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
.DetailT-4-enter-active {
  animation: slide-top 0.4s 1.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
.DetailT-4-leave-active {
  animation: slide-top 0.2s 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
.DetailT-5-enter-active {
  animation: slide-top 0.4s 1.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
.DetailT-5-leave-active {
  animation: slide-top 0.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) both reverse;
}
