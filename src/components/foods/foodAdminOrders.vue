      ...
      <transition name="DetailT-1">
        <div class="order">
          ...
          <div class="detailimgs">
            <img src="https://img95.699pic.com/photo/50180/5833.jpg_wh300.jpg" alt="" />
          </div>
...
export default {
  ...
  methods: {
    ...
    changStatus() {
      if (this.allData.orderStatus == '已下单') {
        ...
        this.$axios.post('/api/updateFoodOrders', { orderId: this.allData.orderId, orderStatus: '烹饪中' }).then(
          (response) => {
        ...
        this.isFinish = true
        // eslint-disable-next-line no-unused-vars
        this.$axios.post('/api/updateFoodOrders', { orderId: this.allData.orderId, orderStatus: '已完成' }).then(
          (response) => {
    ...
    everyTimeCheck() {
      ...
      var time = 0
      var price = 0
      this.$axios.post('/api/selectFoodOrdersById', { orderId: this.allData.orderId }).then(
        (response) => {
  ...
  mounted() {
    this.$bus.$on('adminOrderShow', (data) => {
      ...
      this.$axios.post('/api/selectFoodOrdersById', { orderId: data }).then(
        (response) => {
